...
- **🟦 Standard — operational reporting** for the people *running* the contact center (supervisors, admins). Answers *"How is my center running?"* — queues, SLA, handle time, agent activity. Native Flex Insights surfaced cleanly in the Connie UI. Included with every account.
- **🟪 Advanced — outcomes for external audiences** (funders, county, board, grants). Answers *"What impact are we having, and can I hand it to a funder?"* The connie.plus custom layer pulls your data out of Insights via API and reformats it into funder-ready deliverables. Native Insights produces operational dashboards; it does **not** produce funder-ready impact reports. Closing that gap is the entire reason Advanced exists.

Within Advanced there are two kinds of work:
...
The **Flex Insights API is the seam** between the tiers: Standard is *"look at the dashboards"*; Advanced is *"we pull your data and make it funder-ready."*

:::caution Tier = purpose, not where it's built
A report's tier is set by **who it's for and why**, *not* by which tool produced it. A community-impact dashboard built inside native Insights today is still **Advanced** — its purpose is an external-audience outcome. Don't let "it's built in Insights" pull Impact back into Standard.
:::
...
The defining capability of the Datacenter. Every report lets you move from the **top-level number** down to the **single conversation** behind it:
...
:::tip Seeing SLA at 0%?
A queue showing **0%** SLA usually means **no threshold is configured yet**, or every task ran past it. SLA targets are **not** set in Connie's config files (those only control which SLA tiles display and the Teams handle-time thresholds) — they're set in the admin UI. See **[Administrators → Standard Reporting](/connie-datacenter/administrators/basic)** to configure Service Levels.
:::
...
|---|---|---|
| **Real-Time** (live dashboards) | 🟦 Standard | Operational — running the center |
| **Historical** (adhoc dashboards & reports) | 🟦 Standard | Operational — trends over time |
...
Abandoned Conversations % · Abandon Time · Advanced · Agent · Agent Location · Agent Role · Agent Team · AvgTalk · AvgWrap · Channel · Department · Drill-Down · Experience Time · Follow-up · Handled · Handling Department · Handling Time · Historical · Hold Time · Impact · KPI · Missed · Queue · Queue Time · Quality % · Real-time · Rejected · Service Level (SLA) · Short Abandoned Time · SLA Threshold · Standard · Talk Time · Task Lifecycle · Wrap Time
