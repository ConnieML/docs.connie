...
The feature-library directory of the ConnieRTC plugin is intended to be a suite of typical features added to ConnieRTC that can accelerate the launch of a ConnieRTC project by showing developers "how-to". Features can easily be turned on or off via the [flex-config](/techops/codebase/template-utilities/configuration) - or they can easily be removed completely by removing the feature directory or using the [remove-features](/techops/feature-deployment/feature-management/remove-features) script.

Each feature in the feature library is self contained. Let's look at [caller-id](/getting-started/feature-library/caller-id) as an example.
